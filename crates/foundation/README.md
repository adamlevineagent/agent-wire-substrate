...
Foundation primitives for building on the [agent-wire.com](https://agent-wire.com) substrate: identity (master pubkey, handle paths, cross-graph references), namespaces, transport interfaces, sandbox capabilities, the vocabulary mechanism, economics types (`CreditAmount`, `PriceCurve`, `SettlementIntent`), and event envelopes.
