...
use agent_wire_foundation::{
    ...
    LocalStateSubjectKind, TopicTag, WireNativeDocCodec,
};
    ...
    pub fn new(state_dir: impl AsRef<Path>) -> Self {
        Self {
            state_file: state_dir
                .as_ref()
                .join(LocalStateRecordKind::IdentitySnapshot.directory_name())
                .join("v1-identity.md"),
        }
    }
    ...
    pub fn persist(
        ...
        let mut document = codec
            .document(
                ...
                LocalStateRecordId::new("v1-identity")
                    .map_err(|error| format!("failed to build identity record id: {error}"))?,
                LocalStateSchema::IdentitySnapshotV1,
                persisted.clone(),
                "# Operator Notes\n\nV1 node identity snapshot generated by agent-wire-substrate-node.\n",
            )
            .map_err(|error| format!("failed to build identity state doc: {error}"))?;
        document.frontmatter.topics = vec![
            TopicTag::new("agent-wire-substrate-node")
                .map_err(|error| format!("failed to build identity state topic: {error}"))?,
            TopicTag::new("identity")
                .map_err(|error| format!("failed to build identity state topic: {error}"))?,
        ];
        document.frontmatter.subjects = vec![LocalStateSubject {
            kind: LocalStateSubjectKind::WireHandle,
            value: persisted.operator.clone(),
        }];
        codec
            .write(&self.state_file, &document)
    ...
    pub fn load(&self) -> Result<V1PersistedIdentity, String> {
        if self.state_file.exists() {
            let document = WireNativeDocCodec::new()
                .read::<V1PersistedIdentity>(&self.state_file)
                .map_err(|error| format!("failed to decode identity state doc: {error}"))?;
            return Ok(document.payload);
        }
        let legacy_file = self
            .state_file
            .parent()
            .and_then(Path::parent)
            .map(|state_dir| state_dir.join("v1-identity.json"))
            .ok_or_else(|| "failed to resolve legacy identity state path".to_owned())?;
        let body = fs::read_to_string(&legacy_file)
            .map_err(|error| format!("failed to read identity state: {error}"))?;
        serde_json::from_str(&body)
            .map_err(|error| format!("failed to decode legacy identity state JSON: {error}"))
    }
}
